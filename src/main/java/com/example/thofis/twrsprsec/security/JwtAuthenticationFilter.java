...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public static final String JWT_COOKIE_NAME = "SESSION_TOKEN";
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
		...
		log.info("storing jwt in a cookie");
		storeCookie(JWT_COOKIE_NAME, token, response, request);
	}

	private void storeCookie(String jwtCookieName, String token, HttpServletResponse response, HttpServletRequest request) {
		CookieGenerator cookieGenerator = new CookieGenerator();
		cookieGenerator.setCookieName(jwtCookieName);
		cookieGenerator.setCookieHttpOnly(true);
		// cookie should live until client session terminates
